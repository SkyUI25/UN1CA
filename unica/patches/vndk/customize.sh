if [[ "$SOURCE_VNDK_VERSION" != "$TARGET_VNDK_VERSION" ]]; then
    if $TARGET_OS_BUILD_SYSTEM_EXT_PARTITION == "true"; then
    ...
    if [ ! -f "$SYS_EXT_DIR/apex/com.android.vndk.v$TARGET_VNDK_VERSION.apex" ]; then
        ...
        case "$TARGET_VNDK_VERSION" in
            ...
            "31")
                ...
                ;;
            "33")
                ADD_TO_WORK_DIR "m21xnsxx" "system" "system_ext/apex/com.android.vndk.v33.apex" 0 0 644 "u:object_r:system_file:s0"
                ;;
            "34")
                ADD_TO_WORK_DIR "r12sxxx" "system_ext" "apex/com.android.vndk.v34.apex" 0 0 644 "u:object_r:system_file:s0"
                ;;
        esac
		LOG_STEP_IN "- Change VNDK $SOURCE_VNDK_VERSION to $TARGET_VNDK_VERSION."
        if $NO_APEX; then
